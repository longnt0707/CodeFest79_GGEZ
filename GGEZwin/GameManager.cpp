...
{
	socketioStartConnect(UNIQUE_SERVER_ADDRESS);

	socketioRegisterEvent(PLAYER_INCOMMING_ROOM_STATE, OnIncommingRoomState);
}

void GameManager::SocketIOJionRoom(std::string matchID)
{
	//socketioJoinRoom(UNIQUE_TEAM_ID, matchID, PLAYER_OUTGOING_JOIN_ROOM);
	socketioJoinRoom(UNIQUE_TEAM_ID, UNIQUE_MATCH_ID_TRAINING, PLAYER_OUTGOING_JOIN_ROOM);
}
